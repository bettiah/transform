import { HistoryVisibilityEvent } from './client-server/events';
import { validate } from 'class-validator';
import { existsInRedis, RedisKeys, redisEnque } from './redis';

const debug = require('debug')('server:events:historyVisibility');

export async function handleHistroyVisisbility(event: HistoryVisibilityEvent) {
  if (!(event instanceof HistoryVisibilityEvent)) {
